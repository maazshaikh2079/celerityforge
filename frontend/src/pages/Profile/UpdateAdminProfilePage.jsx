...
import adminService from "../../services/admin.service.js"; // Adjust path as necessary
...
const UpdateAdminProfilePage = () => {
  ...
  const { adminToken, adminData, setAdminData } = useContext(AdminContext);
  ...
  useEffect(() => {
    if (adminData) {
      ...
      if (
        ...
        !adminData.profile_image_url.includes("default-avatar")
      ) {
        setImagePreview(adminData.profile_image_url);
      }
    }
  }, [adminData]);
  ...
  const handleUpdateProfileSubmit = async (e) => {
    ...
    try {
      ...
      const updatedAdminData = data.admin_data;

      // Update local context so the header/sidebar reflect changes immediately
      if (updatedAdminData) {
        setAdminData({ ...adminData, ...updatedAdminData });
      }
      ...
      <div className="flex flex-col gap-4">
        ...
        <div>
          ...
          <p className="text-body-sm text-neutral">
            Modify your identity specifications and security credentials
          </p>
        </div>
      </div>
